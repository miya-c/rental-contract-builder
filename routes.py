...
def edit_building(building_id):
    ...
    roof_structure_values = [choice[0] for choice in form.roof_structure.choices]
...
def view_building(building_id):
    ...
    return render_template('buildings/view.html', building=building, rooms=rooms)
...
def add_room(building_id):
    ...
    if form.validate_on_submit():
        ...
        if form.custom_amenities.data:
            custom_amenities = [item.strip() for item in form.custom_amenities.data.split(',') if item.strip()]
        ...
        room = Room(
            ...
            custom_amenities=json.dumps(custom_amenities) if custom_amenities else None,
            notes=form.notes.data,
...
def edit_room(room_id):
    ...
    if form.validate_on_submit():
        ...
        room.custom_amenities = json.dumps(custom_amenities) if custom_amenities else None
...
def delete_special_term(term_id):
    ...
    stmt = contract_special_terms.delete().where(contract_special_terms.c.special_term_id == term_id)
...
def create_contract():
    ...
    if form.validate_on_submit():
        ...
        count = Contract.query.filter(Contract.contract_number.like(f"{current_date}%")).count() + 1
...
def download_contract_pdf(contract_id):
    ...
    return send_file(contract.pdf_path, as_attachment=True, download_name=filename)
...
def api_room_details(room_id):
    ...
    room_data = {
        ...
        'building': {
            ...
            'roof_structure': building.roof_structure,
            'floors': building.floors,
            'building_type': building.building_type,
            'construction_date': building.construction_date.strftime('%Y-%m-%d') if building.construction_date else None
        },
        'owner': {
            'id': owner.id,
            'name': owner.name,
...
def api_agent_details(agent_id):
    ...
    agent_data = {
        ...
        'registration_date': agent.registration_date.strftime('%Y-%m-%d') if agent.registration_date else None
    }
...
def page_not_found(e):
    return render_template('error.html', error_code=404, error_message="ページが見つかりませんでした。"), 404
...
def forbidden(e):
    return render_template('error.html', error_code=403, error_message="このページにアクセスする権限がありません。"), 403
