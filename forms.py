...
class RealEstateAgentForm(FlaskForm):
    ...
    license_number = StringField('免許番号',
                                 validators=[DataRequired(),
                                             Length(max=50)])
    registration_date = DateField('登録日',
                                  validators=[Optional()],
                                  format='%Y-%m-%d')
    ...
    def validate_license_number(self, field):
        ...
        agent = RealEstateAgent.query.filter_by(
            license_number=field.data).first()
...
class BuildingForm(FlaskForm):
    ...
    structure_choices = [('木造', '木造'), ('コンクリートブロック造', 'コンクリートブロック造'),
                         ('鉄骨造', '鉄骨造'), ('鉄筋コンクリート造', '鉄筋コンクリート造'),
                         ('木骨石造', '木骨石造'), ('軽量鉄骨造', '軽量鉄骨造'),
                         ('その他', 'その他（自由入力）')]
    structure = SelectField('構造',
                            choices=structure_choices,
                            validators=[DataRequired()])
    custom_structure = StringField('その他の構造',
                                   validators=[Optional(),
                                               Length(max=50)])

    roof_structure_choices = [('陸屋根', '陸屋根'), ('瓦葺き', '瓦葺き'),
                              ('スレート葺き', 'スレート葺き'), ('亜鉛メッキ合板葺き', '亜鉛メッキ合板葺き'),
                              ('草葺き', '草葺き'), ('セメント瓦葺き', 'セメント瓦葺き'),
                              ('アルミニューム葺き', 'アルミニューム葺き'), ('板葺き', '板葺き'),
                              ('杉皮葺き', '杉皮葺き'), ('ビニール板葺き', 'ビニール板葺き'),
                              ('その他', 'その他（自由入力）')]
    roof_structure = SelectField('屋根構造',
                                 choices=roof_structure_choices,
                                 validators=[Optional()])
    custom_roof_structure = StringField(
        'その他の屋根構造', validators=[Optional(), Length(max=50)])

    floors = IntegerField('階数', validators=[DataRequired()])
    total_units = IntegerField('総戸数', validators=[DataRequired()])

    building_type_choices = [('アパート', 'アパート'), ('マンション', 'マンション'),
                             ('戸建て', '戸建て'), ('貸間', '貸間'),
                             ('その他', 'その他（自由入力）')]
    building_type = SelectField('建物の種類',
                                choices=building_type_choices,
                                validators=[DataRequired()])
    custom_building_type = StringField('その他の建物種類',
                                       validators=[Optional(),
                                                   Length(max=50)])

    construction_date = DateField('新築年月',
                                  validators=[Optional()],
                                  format='%Y-%m-%d')
    notes = TextAreaField('備考', validators=[Optional(), Length(max=1000)])
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.owner_id.choices = [(owner.id, owner.name)
                                 for owner in Owner.query.all()]


class RoomForm(FlaskForm):
    """Room information form"""
    room_number = StringField('部屋番号',
                              validators=[DataRequired(),
                                          Length(max=20)])
    ...
    # Custom amenities
    custom_amenities = TextAreaField('追加設備（カンマ区切りで入力）',
                                     validators=[Optional()])
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.building_id.choices = [(building.id, building.name)
...
class ContractForm(FlaskForm):
    ...
    tenant_name = StringField('借主名',
                              validators=[DataRequired(),
                                          Length(max=100)])
    tenant_address = StringField('借主住所',
                                 validators=[DataRequired(),
                                             Length(max=255)])
    tenant_phone = StringField('借主電話番号',
                               validators=[Optional(),
                                           Length(max=20)])
    tenant_email = StringField(
        '借主メールアドレス', validators=[Optional(),
                                 Email(), Length(max=120)])

    start_date = DateField('契約開始日',
                           validators=[DataRequired()],
                           format='%Y-%m-%d')
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.room_id.choices = [(room.id,
                                 f"{room.building.name} {room.room_number}")
                                for room in rooms]

        # Populate the agent dropdown
        self.agent_id.choices = [(agent.id,
                                  f"{agent.name} ({agent.license_number})")
...
class ContractTemplateForm(FlaskForm):
    ...
    file_type = SelectField('ファイル形式',
                            choices=[('html', 'HTML'), ('excel', 'Excel'),
                                     ('word', 'Word'), ('pdf', 'PDF')],
                            validators=[DataRequired()])

    template_file = FileField('テンプレートファイルをアップロード', validators=[Optional()])
    ...
    def validate_template_file(self, field):
        ...
        if field.data:
            ...
            if self.file_type.data == 'excel' and not (
                    filename.endswith('.xlsx') or filename.endswith('.xls')):
                raise ValidationError(
                    'Excel形式のファイル(.xlsx, .xls)をアップロードしてください。')
            elif self.file_type.data == 'word' and not (
                    filename.endswith('.docx') or filename.endswith('.doc')):
                raise ValidationError('Word形式のファイル(.docx, .doc)をアップロードしてください。')
            elif self.file_type.data == 'pdf' and not filename.endswith(
                    '.pdf'):
